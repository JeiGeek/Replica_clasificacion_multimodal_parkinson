...
######### Extract head embeddings
def save_head_embeddings(sam_model, subject_id, base_dir="embeddings_heads"):
    import os
    import numpy as np
    import torch
    ...
    for blk in sam_model.image_encoder.blocks:
        if hasattr(blk.attn, "last_head_embeddings"):
            head_embeds.append(
                blk.attn.last_head_embeddings.cpu().numpy()
            )

    if len(head_embeds) == 0:
        return

    head_embeds = np.stack(head_embeds, axis=0)  # [L, H, D]

    num_heads = head_embeds.shape[1]

    for h in range(num_heads):
        head_dir = os.path.join(base_dir, f"head_{h:02d}")
        os.makedirs(head_dir, exist_ok=True)

        np.save(
            os.path.join(head_dir, f"{subject_id}.npy"),
            head_embeds[:, h, :]  # [L, D_head]
        )
...
def build_model(args):
    ...
    sam_model.mask_decoder.return_features = True
    for blk in sam_model.image_encoder.blocks:
        blk.attn.return_head_embeddings = True
...
class BaseTrainer:
    ...
    def batch_forward(self, sam_model, image_embedding, gt3D, low_res_masks, points=None, subject_id=None, epoch=None, step=None):
        """
        Realiza la llamada al prompt encoder + mask_decoder.
        Devuelve (low_res_masks, prev_masks, decoder_features_or_None).
        """
        ...
        decoder_output = sam_model.mask_decoder(
        ...
        # ===== SAVE HEAD EMBEDDINGS (ONCE PER VOLUME) =====
        if subject_id is not None:
            save_head_embeddings(sam_model, subject_id)
        



        # Puede devolver (masks, iou) o (masks, iou, features)
        if isinstance(decoder_output, tuple):
            if len(decoder_output) == 4:
                low_res_masks, iou_predictions, decoder_features, _ = decoder_output
            elif len(decoder_output) == 3:
                low_res_masks, iou_predictions, decoder_features = decoder_output
            else:
                low_res_masks, iou_predictions = decoder_output[:2]
                decoder_features = None
        else:   
            low_res_masks, iou_predictions = decoder_output
            decoder_features = None
        ...
        prev_masks = F.interpolate(low_res_masks,
                                   ...
                                   mode='trilinear',
                                   align_corners=False)
        if hasattr(sam_model.mask_decoder, "last_fused_embedding"):
                fused = sam_model.mask_decoder.last_fused_embedding
                fused = fused.mean(dim=(2,3,4))  # pooling espacial
                np.save(
                    f"embeddings_fused/{subject_id}.npy",
                    fused.cpu().numpy()
                )

        #if decoder_features is not None:
            #print(f"[DEBUG] Keys in decoder_features: {list(decoder_features.keys())}")
        #else:
            #print("[DEBUG] decoder_features is None")

        # save embedding if exists
        if decoder_features is not None and "upscaled_embedding" in decoder_features:
            upscaled_embed = decoder_features["upscaled_embedding"].detach().cpu().numpy()
            os.makedirs("embeddings_decoder_mri_brain-stem", exist_ok=True)
            #np.save(f"embeddings_decoder_train_test/{subject_id}_e{epoch}_s{step}.npy", upscaled_embed)
            np.save(f"embeddings_decoder_mri_brain-stem/{subject_id}.npy", upscaled_embed)

            

        return low_res_masks, prev_masks, decoder_features
    ...
    def interaction(self, sam_model, image_embedding, gt3D, num_clicks, subject_id=None, epoch=None, step=None):
        return_loss = 0
        ...
        for num_click in range(num_clicks):
            ...
            # llamamos siempre esperando 3 valores (el tercero puede ser None)
            if num_click == random_insert or num_click == num_clicks - 1:
                low_res_masks, prev_masks, decoder_features = self.batch_forward(
                    sam_model,
                    image_embedding,
                    gt3D,
                    low_res_masks,
    ...
    def train_epoch(self, epoch, num_clicks):
        ...
        for step, data3D in enumerate(tbar):
            try:
                image3D, gt3D, image_path = data3D["image"], data3D["label"], data3D["path"]

                if "path" in data3D:
                    paths = data3D["path"]
                    if isinstance(paths, (list, tuple)):
                        for p in paths:
                            subject_id = os.path.basename(p).split("_")[1]
                            with torch.no_grad():
                                embedding = sam_model.image_encoder(image3D.to(device))
                            os.makedirs("embeddings_mri_all_full_pd", exist_ok=True)
                            # np.save(f"embeddings_mri_all_full_pd/{subject_id}.npy", embedding.cpu().numpy())
                    else:
                        subject_id = os.path.basename(paths).split("_")[1]
                        with torch.no_grad():
                            embedding = sam_model.image_encoder(image3D.to(device))
                        os.makedirs("embeddings_mri_all_full_pd", exist_ok=True)
                        np.save(f"embeddings_mri_all_full_pd/{subject_id}.npy", embedding.cpu().numpy())
...
def main():
    ...
    if args.multi_gpu:
        ...
        model.mask_decoder.return_features = True
        # Create trainer
